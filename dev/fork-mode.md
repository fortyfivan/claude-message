...
2. **Step 2** — Remove plugin root discovery entirely:
   ...
   - Replace with: "Read current skills from `.claude/skills/`." For each skill, assess tuning state via `metadata.tuned: true` in frontmatter.
...
5. **Manual Edit Detection section** — Remove comparison against plugin base. Instead, detect manual edits by checking if the file has been modified since `metadata.tuned_date` (via git or file metadata). Simplify: warn the user that re-tuning will overwrite any manual changes to tuned sections.
