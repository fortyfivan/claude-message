...
| `plan_slice` | Resolved decisions + raw material for THIS document only (positioning components, UVP lines with measures + proof tags, per-persona load-bearing specs, cross-tags; for assets/variants: content-keys schema, production-targets, conventions, the variant's when-to-use trigger, voice-shift note, the **full structure anatomy** — ordered sections each with purpose + length band — CTA pattern, writing-check bullets). Primary input. Decisions are fixed; prose is yours to render. |
